...
from .destinations.redshift import RedshiftLoader
from .systems.util import schema_validation, validate_kwargs
    ...
    def load_warehouse(self, **kwargs):
        ...
        for collection in self.data.keys():
            if collection != "metadata":
                loader = RedshiftLoader(engine)
                ok, message, new_incoming_columns = loader._insert_update_record(
        ...
        if self.mongo_conn:
            for collection in successful_tables:
                MongoRunStateStore(self.mongo_conn).log_run(
                    collection,
                    {"status": "success", "loader": True, "latest_record_updated_value": str(dt.now())},
                )
        engine.dispose()
        return outcome
