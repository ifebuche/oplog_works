...
def s3_upload(tableName, data, year=year, month=month, day=day):
    """
...
def insert_update_record(engine, df, targetTable, pk):
    """
...
def redshift_s3_write(tableName, table):
    """
    ...
    try:
        ...
        s3_upload(tableName, table_2_s3)
    except Exception as e:
...
#    write to RedshiftConn
    try:
        if tableName == 'requests':
            RedshiftConn_inserted, msg = insert_update_record(engine, table, f"{tableName}", 'requestid')
            if RedshiftConn_inserted:
                print("RS insert successful")
        elif tableName == 'trips':
            RedshiftConn_inserted, msg = insert_update_record(engine, table, f"{tableName}", 'tripid')
            if RedshiftConn_inserted:
                print("RS insert successful")
        elif tableName == 'invoice_freight_requests':
            RedshiftConn_inserted, msg = insert_update_record(engine, table, f"{tableName}", 'freightrequestid')
            if RedshiftConn_inserted:
                print("RS insert successful")
        else:  
            RedshiftConn_inserted, msg = insert_update_record(engine, table, f"{tableName}", '_id')
            if RedshiftConn_inserted:
                print("RS insert successful")
    except Exception as e:
        # capture_exception(e)
