...
try:
    ...
    certifi = pymongo = load_dotenv = randint = create_engine = inspect = pd = None

INTEGRATION_DEPS = all(x is not None for x in (certifi, pymongo, pd, create_engine))
...
def _integration_enabled():
    return INTEGRATION_DEPS and bool(os.getenv("mongo_conn")) and bool(os.getenv("host"))


@unittest.skipUnless(_integration_enabled(), "set mongo_conn and postgres env vars for integration tests")
class TestDataExtractionIntegration(unittest.TestCase):
    ...
    def test_data_extraction(self):
        ...
        print(final)
        loader = Loader(mongo_conn=self.mongo_client, data=final, datalake=False, datawarehouse=True, aws={})
        loader.run(
            host=os.getenv("host"),
            user=os.getenv("user"),
