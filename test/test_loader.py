...
class TestDataLoader(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.name = "user908" #"user" + str(randint(25,2000))
        print(cls.name)
    ...
    def test_loader_fresh_table(self):
        print('--------------FRESH------------------')
        self.loader.run(warehouse=True,host=os.getenv('host'),user=os.getenv('user'),password=os.getenv('password'),db=os.getenv('db'),port=5432)
    ...
    def test_loader_updated_table(self):
        ...
        self.loader.run(schema_on_conflict='PASS' ,warehouse=True,host=os.getenv('host'),user=os.getenv('user'),password=os.getenv('password'),db=os.getenv('db'),port=5432)
    ...
    def test_loader_new_column_fail(self):
        ...
        self.loader = Loader(self.client,self.final)
        try:
            self.loader.run(schema_on_conflict='FAIL' ,warehouse=True,host=os.getenv('host'),user=os.getenv('user'),password=os.getenv('password'),db=os.getenv('db'),port=5432)
            #raise a test fail
        except:
            #test pass
            pass
